...
- **Customização de Componentes**: O arquivo `flutter/lib/desktop/widgets/button.dart` foi modificado para suportar `bgColor` customizado no `FixedWidthButton` nas branches de homologação (necessário para pintar o banner do UAC em Verde Claro no `desktop_home_page.dart` sem depender inteiramente do *ThemeData* ou forçar erro de compilação por falta de parâmetro nomeado). Em caso de atualizações (merges) da versão original do RustDesk, lembre-se de re-inserir o atributo `bgColor` no widget se ele for sobrescrito.
